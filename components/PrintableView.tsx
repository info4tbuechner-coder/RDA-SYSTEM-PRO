...
const PrintableView: React.FC<PrintableViewProps> = ({ result }) => {
    ...
    return (
        <div style={{ 
            ...
            {/* Methodik & Herleitung (Gutachten-Standard) */}
            <section style={{ marginBottom: '40px', border: '1px solid #cbd5e1', padding: '24px', borderRadius: '12px', backgroundColor: '#fff' }}>
                <h2 style={{ fontSize: '13px', fontWeight: '800', color: '#1e293b', textTransform: 'uppercase', marginBottom: '8px', letterSpacing: '1px', fontFamily: isCourtMode ? "'Georgia', serif" : 'inherit' }}>
                    Gutachterlicher Prüfpfad & Auswertungskriterien
                </h2>
                <div style={{ fontSize: '9px', fontWeight: '700', color: '#0ea5e9', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '15px' }}>
                    Wissenschaftlicher Prüf- & Neutralitätsstandard | FamCOMM-Eval v5.2
                </div>
                
                {/* Steps indicator */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '10px', marginBottom: '24px', backgroundColor: '#f8fafc', padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    {(result.methodische_herleitung?.verfahrensschritte || [
                        "1. Primärscan",
                        "2. Kriterienabgleich",
                        "3. Entlastungsevaluierung",
                        "4. Quantifizierung",
                        "5. BIFF-Formulierung"
                    ]).map((schritt, idx) => (
                        <div key={idx} style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '9px', fontWeight: '800', color: '#64748b', marginBottom: '4px' }}>SCHRITT 0{idx + 1}</div>
                            <div style={{ fontSize: '10px', fontWeight: '600', color: '#0f172a', lineHeight: '1.2' }}>{schritt.replace(/^\d+\.\s*/, '')}</div>
                        </div>
                    ))}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '24px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        <div>
                            <h4 style={{ margin: '0 0 6px', fontSize: '11px', fontWeight: '700', color: '#0f172a' }}>1. Wissenschaftliches Auswertungsverfahren</h4>
                            <p style={{ margin: 0, fontSize: '11px', color: '#334155', lineHeight: '1.5' }}>
                                {result.methodische_herleitung?.analyse_verfahren || "Qualitativ-forensische Inhaltsanalyse nach psychologischen Standards. Auswertung syntaktischer Druckmittel, semantischer Verschleierungstechniken sowie dysfunktionaler interpersoneller Kontrollbestrebungen."}
                            </p>
                        </div>
                        <div>
                            <h4 style={{ margin: '0 0 6px', fontSize: '11px', fontWeight: '700', color: '#0f172a' }}>2. Sachlichkeit & Objektivierung</h4>
                            <p style={{ margin: 0, fontSize: '11px', color: '#334155', lineHeight: '1.5' }}>
                                {result.methodische_herleitung?.sachlichkeits_garantie || "Das Verfahren schließt haltlose Zuschreibungen oder unbewiesene Persönlichkeitsdiagnosen aktiv aus. Klassifizierungen werden ausschließlich durch unmittelbar korrelierende, wörtliche Zitate belegt."}
                            </p>
                        </div>
                        <div>
                            <h4 style={{ margin: '0 0 4px', fontSize: '11px', fontWeight: '700', color: '#0f172a' }}>3. Entlastungsanalyse & Neutralitätsprüfung</h4>
                            <p style={{ margin: 0, fontSize: '11px', color: '#475569', lineHeight: '1.4', fontStyle: 'italic', borderLeft: '2px solid #0ea5e9', paddingLeft: '8px' }}>
                                {result.methodische_herleitung?.entlastende_faktoren || "Ein systematischer Modusscan lieferte keine Anzeichen deeskalierender oder aufrichtig kooperativer Textpassagen im Input."}
                            </p>
                        </div>
                    </div>
                    <div style={{ backgroundColor: '#f8fafc', padding: '18px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                        <h4 style={{ margin: '0 0 10px', fontSize: '10px', fontWeight: '700', color: '#475569', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Geprüfte Kriterien</h4>
                        <ul style={{ margin: '0 0 15px', paddingLeft: '15px', fontSize: '10px', color: '#1e293b', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            {((result.methodische_herleitung?.kriterien_anwendung) || [
                                "Kooperationsbereitschaft im Wohl des Kindes",
                                "Sicherung partnerschaftlicher Grenzen",
                                "Abgrenzung destruktiver Loyalitätsansprüche",
                                "Linguistische Bindungsfürsorge",
                                "Schuldbeilegungseignung"
                            ]).map((kriterium, i) => (
                                <li key={i} style={{ lineHeight: '1.3', fontWeight: '500' }}>{kriterium}</li>
                            ))}
                        </ul>
                        <div style={{ borderTop: '1px solid #cbd5e1', paddingTop: '12px' }}>
                            <h5 style={{ margin: '0 0 4px', fontSize: '9px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' }}>Bewertungs-Herleitung</h5>
                            <p style={{ margin: 0, fontSize: '10px', color: '#475569', lineHeight: '1.4' }}>
                                {result.methodische_herleitung?.objektive_logik || "Bewertung beruht auf der relativen Dichte destruktiver Interaktionsereignisse."}
                            </p>
                        </div>
                    </div>
                </div>
