...
export const PatternSummaryGrid: React.FC<PatternSummaryGridProps> = ({
    ...
    // Detect mobile for slide-up drawer info panel
    React.useEffect(() => {
    ...
    return (
        <div id="tactical-threat-matrix" className="space-y-6 md:space-y-10">
            ...
            <AnimatePresence>
                {isMobile && isInspectDrawerOpen && selectedPattern && (
                    <motion.div 
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[1000] bg-black/80 backdrop-blur-md flex items-end justify-center"
                        onClick={() => setIsInspectDrawerOpen(false)}
                    >
                        <motion.div 
                            initial={{ y: "100%" }}
                            animate={{ y: 0 }}
                            ...
                            className="bg-[#050b16] border-t border-slate-850 rounded-t-[2.5rem] w-full max-h-[85vh] overflow-y-auto p-6 space-y-6 shadow-2xl relative"
                            onClick={(e) => e.stopPropagation()}
                        >
                            {/* Drawer Drag handle indicator */}
                            <div className="w-12 h-1.5 bg-slate-800 rounded-full mx-auto mb-2 opacity-60"></div>
                            
                            <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                    <span className="text-[8px] font-black text-brand-primary uppercase tracking-[0.3em] font-mono flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 bg-brand-primary rounded-full animate-ping" />
                                        Tactical Inspect (Mobile HUD)
                                    </span>
                                ...
                                <button 
                                    onClick={() => setIsInspectDrawerOpen(false)}
                                    className="p-2 border-2 border-slate-800 text-slate-400 rounded-full bg-slate-950 font-black text-xs h-9 w-9 flex items-center justify-center hover:text-white"
                                >
                                    ✕
                                </button>
                            </div>
                            ...
                            {/* Evidence Quoted Content */}
                            <div className="space-y-2">
                                <span className="text-[8px] font-mono text-slate-500 uppercase tracking-widest block font-bold">Linguistischer Payload (Zitat)</span>
                                <div className="bg-slate-950/80 border-l-2 border-brand-accent p-4 rounded-r-xl relative overflow-hidden">
                                    <p className="font-serif italic text-sm text-slate-200 leading-relaxed">
                                        "{selectedPattern.zitat}"
                                    </p>
                            ...
                            {/* Action Shortcuts */}
                            <div className="pt-4 border-t border-slate-900/50 flex gap-3">
                                <button
                                    onClick={() => {
                                        setIsInspectDrawerOpen(false);
                                        onLocatePattern(selectedPattern.id);
                                    }}
                                    className="flex-1 py-3 px-4 bg-brand-primary/10 border border-brand-primary/30 text-brand-primary hover:bg-brand-primary hover:text-white rounded-xl text-[10px] font-black uppercase tracking-wider font-mono text-center transition-all"
                                >
                                    🔍 Scroll Mirror
